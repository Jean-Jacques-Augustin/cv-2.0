...
export const navigationLinks = [{name: "Home", path: "/", icon: <HomeRoundedIcon/>}, {
    name: "Profil",
    path: "/profil",
...
const ListesLinks = () => {
    return (<Box sx={{mt: 9}}>
        <List>
            {navigationLinks.map((item, index) => (<ListItem key={index} disablePadding>
                <ListItemButton
                    className="listLinks"
                    component={Link}
                    to={item.path}
            ...
            <Toolbar className="navigation">
                ...
                <Hidden mdDown>
                    <div className={"nav_item"}>
                        {navigationLinks.map((item, key) => (<Typography
                                component={Link}
                                to={item.path}
                                key={key}
                                variant="h6"
                                sx={{
                                    ml: 2, color: 'text.secondary', textDecoration: 'none',
                ...
                <Drawer
                    ...
                    sx={{
                        width: drawerWidth, flexShrink: 0, [`& .MuiDrawer-paper`]: {
                            width: drawerWidth, boxSizing: "border-box", border: 0,
                        },
                    }}
                    anchor={"left"}
                    open={open}
